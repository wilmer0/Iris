    ...
    public class modeloCxpNotaCredito
    {
        ...
        public bool agregarNotaCredito(cxp_nota_credito notaCredito)
        {
            try
            {
                ...
                sql = "insert into cxp_nota_credito(codigo,codigo_suplidor,fecha,activo,codigo_empleado,monto,detalle,codigo_compra,codigo_concepto,codigo_devolucion) values('" + notaCredito.codigo + "','" + notaCredito.codigoSuplidor + "'," + utilidades.getFechayyyyMMdd(notaCredito.fecha) + ",'" + activo.ToString() + "','" + notaCredito.codigoEmpleado + "','" + notaCredito.monto + "','" + notaCredito.detalle + "','" + notaCredito.codigoCompra + "','" + notaCredito.codigoConcepto + "','" + notaCredito.codigoDevolucion + "')";
                //MessageBox.Show(sql);
        ...
        public bool modificarNotaCredito(cxp_nota_credito notaCredito)
        {
            try
            {
                ...
                sql = "update cxp_nota_credito set codigo_suplidor='" + notaCredito.codigoSuplidor + "',fecha='" + utilidades.getFechayyyyMMdd(notaCredito.fecha) + "',activo='" + activo.ToString() + "',codigo_empleado='" + notaCredito.codigoEmpleado + "',monto='" + notaCredito.monto + "',detalle='" + notaCredito.detalle + "',codigo_compra='" + notaCredito.codigoCompra + "',codigo_concepto='" + notaCredito.codigoConcepto + "',codigo_devolucion='" + notaCredito.codigoDevolucion + "' where codigo='" + notaCredito.codigo + "'";
                ds = utilidades.ejecutarcomando_mysql(sql);
                //MessageBox.Show(sql);
                return true;
            }
        ...
        public cxp_nota_credito getNotaCreditoById(int id)
        {
            try
            {
                ...
                string sql = "select codigo,codigo_suplidor,fecha,activo,codigo_empleado,monto,detalle,codigo_compra,codigo_concepto,codigo_devolucion from cxp_nota_credito where codigo='" + id + "'";
                DataSet ds = utilidades.ejecutarcomando_mysql(sql);
        ...
        public List<cxp_nota_credito> getListaCompleta(bool mantenimiento = false)
        {
            try
            {
                ...
                string sql = "select codigo,codigo_suplidor,fecha,activo,codigo_empleado,monto,detalle,codigo_compra,codigo_concepto,codigo_devolucion from cxp_nota_credito ";
        ...
        public List<cxp_nota_credito> getListaByVentaActivo(int id)
        {
            try
            {
                ...
                List<cxp_nota_credito> lista = new List<cxp_nota_credito>();
                string sql = "select codigo,codigo_suplidor,fecha,activo,codigo_empleado,monto,detalle,codigo_compra,codigo_concepto,codigo_devolucion from cxp_nota_credito where codigo_venta='" + id + "' and activo='1'";
                DataSet ds = utilidades.ejecutarcomando_mysql(sql);
                if (ds.Tables[0].Rows.Count > 0)
                {
                    foreach (DataRow row in ds.Tables[0].Rows)
        ...
        public List<cxp_nota_credito> getListaByVentaActivoAndRangoFechaFinal(int id, DateTime fechaFinal)
        {
            try
            {
                ...
                List<cxp_nota_credito> lista = new List<cxp_nota_credito>();
                string sql = "select codigo,codigo_suplidor,fecha,activo,codigo_empleado,monto,detalle,codigo_compra,codigo_concepto,codigo_devolucion from cxp_nota_credito where fecha<='" + utilidades.getFechayyyyMMdd(fechaFinal) + "' and codigo_compra='" + id + "' and activo='1'";
                DataSet ds = utilidades.ejecutarcomando_mysql(sql);
                if (ds.Tables[0].Rows.Count > 0)
                {
                    foreach (DataRow row in ds.Tables[0].Rows)
        ...
        public List<cxp_nota_credito> getListaBySuplidor(int id)
        {
            try
            {
                ...
                List<cxp_nota_credito> lista = new List<cxp_nota_credito>();
                string sql = "select codigo,codigo_suplidor,fecha,activo,codigo_empleado,monto,detalle,codigo_compra,codigo_concepto,codigo_devolucion from cxp_nota_credito where codigo_suplidor='" + id + "' and activo='1'";
                DataSet ds = utilidades.ejecutarcomando_mysql(sql);
                if (ds.Tables[0].Rows.Count > 0)
                {
                    foreach (DataRow row in ds.Tables[0].Rows)
        ...
        public List<cxp_nota_credito> getListaByRangoFecha(DateTime fechaInicial, DateTime fechaFinal)
        {
            ...
            {
                List<cxp_nota_credito> lista = new List<cxp_nota_credito>();
                string sql = "select codigo,codigo_suplidor,fecha,activo,codigo_empleado,monto,detalle,codigo_compra,codigo_concepto,codigo_devolucion from cxp_nota_credito where fecha>='" + utilidades.getFechayyyyMMdd(fechaInicial.Date) + "' and fecha<='" + utilidades.getFechayyyyMMdd(fechaFinal.Date) + "' and activo='1'";
                DataSet ds = utilidades.ejecutarcomando_mysql(sql);
                if (ds.Tables[0].Rows.Count > 0)
                {
                    foreach (DataRow row in ds.Tables[0].Rows)
